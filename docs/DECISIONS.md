...
- **2026-07-10 (착수 ① 완료) — shoot 렌더 게이트 부활. 아키 변경 0, 픽셀검증 즉시 복귀.** C2 착수순서 ①을 그대로 이행: atelier `shoot.js`를 kiln 게이트 계약에 맞춰 포터블 이식하고 design 스테이지에 외부 게이트로 연결. **끝까지 실제로 돌려 검증**.
  ...
  - **부트스트랩 방식 확정(2026-07-10) — 번들링 아님, "첫 렌더 자동 다운로드".** 논의 결론: 크로미움 바이너리를 build/npm에 넣는 건 **불가**(~150MB×플랫폼별 → 단일 tarball ~1GB, `.next/standalone`은 `spawn`으로 실행되는 `shoot.cjs`를 트레이스도 못 함). 대안 2개 중 **② 자동 다운로드** 채택(① 시스템 Edge/Chrome 채널 재사용은 윈도우 다운로드 0이지만 **버전 제각각 → 픽셀 렌더 불일치**. shoot은 픽셀 게이트라 **모두 동일 크로미움 = 렌더 일관성**이 실질 장점이라 ②를 택함). 규격: 런처(`bin/kiln.js`)가 **첫 렌더가 실제 필요할 때(런처 시작 시 아님 — 렌더까지 안 간 사용자는 안 받음)** `playwright install chromium` 자동 실행 + **"크로미움 받는 중" 진행 표시**(멈춤 오해 방지), **네트워크·프록시 실패 시 graceful skip으로 강등**(오늘 만든 경로 재사용, 크래시 아님), 1회 후 캐시(`AppData/Local/ms-playwright`) 재사용. 사용자는 `npx playwright install`을 **직접 칠 일 없음** — `npx @hb-kit/kiln`만으로 자동.
  - **다음: 착수 ②** — `runAgentic` + claude-code `stream-json` 멀티턴 + hi-fi 빌드 agentic화(shoot=**툴**로 재사용, 오늘 만든 스크립트의 두 번째 용처) + 위 부트스트랩(런처)·standalone 트레이스에 playwright 라이브러리 포함. 상세 [[kiln-c2-agentic]].
- **2026-07-10 (착수 ② 완료) — runAgentic + claude-code stream-json 멀티턴으로 hi-fi 빌드 agentic화(렌더-인-루프 자가수정). A~E 한 세션 관통.** 착수순서 ②를 계획대로 이행하고 **실제 sonnet 에이전트로 end-to-end 검증**.
  - **A. runAgentic 시임(`generate()`의 형제)**: `provider.js`에 `supportsAgentic` 플래그+`runAgentic()` 베이스(‑`supportsStructured` 옆). 신규 `engine/model/agentic.js` = 유일 진입점(`resolveModel`→agentic이면 provider 위임, 아니면 `NOT_AGENTIC` throw로 스테이지가 단발 폴백). `agent-cli.js`에 **`runAgentStream`**(stdout를 개행단위 JSONL 파싱, 줄마다 `onLine(obj)`, cwd/env 전달, 비정상 종료도 `e.events`에 파싱결과 첨부). `claude-code.js`가 `supportsAgentic=true`+`runAgentic` 구현.
  - **B. claude-code stream-json 멀티턴**: `claude -p --output-format stream-json --verbose --strict-mcp-config --max-turns N --permission-mode bypassPermissions --tools Write Read Edit Bash --add-dir <projectDir> [--model][--append-system-prompt]`. task는 stdin. **JSONL 실측 매핑**: `system/init`·`rate_limit_event` 무시 · `assistant.content[]` → text=`turn`, tool_use는 id 기록 · `user.content[]` tool_result → 직전 tool_use와 id 매칭해 **완료된 tool-call 1건**(tool·summary·ok·detail) 발화 · `result`(is_error·subtype·num_turns·usage.{input,output}_tokens)에서 마감.
  - **보안 포스처 정정(실측 발견)**: 처음엔 `--permission-mode acceptEdits`+`--allowedTools "Bash(node scripts/shoot.cjs *)"`로 "임의 Bash 차단"을 의도했으나 — **헤드리스 `-p`에서 allowedTools는 제한이 아님**(default 모드에서도 `echo HELLO_UNSCOPED` 실행됨, `permission_denials:[]`). 게다가 스트릭트한 end-user settings면 **프롬프트로 hang → 파이프라인 붕괴** 위험. → **`bypassPermissions`**로 전환(무조건 비대화, 안 멈춤). 신뢰경계는 그대로(BYO — 사용자가 `claude`를 직접 쓰면 이미 전권). 실질 스코프는 `--tools`(툴셋 한정)·`--strict-mcp-config`(ambient MCP 차단)·`--add-dir`(쓰기 범위)·태스크 프롬프트로. `allowedTools`는 오해 유발이라 체인 전체에서 제거.
  - **C. turn/tool-call 이벤트 5곳 배선**: `events.js` vocab+cliPrinter, `server/types/job.ts` 유니온, `entities/job/types.ts` 미러, `event-row.tsx` 렌더 case 2개. **SSE seam 불변** — 기존 phase/step/gate/artifact 위에 서브이벤트만 얹음(`EventRow`는 미지 type에 null 반환이라 안전).
  - **D. agentic 빌드 스테이지**: `build-screens.js` `buildScreensAgentic`(에이전트가 `<ctx.dir>/screens/*.html` 직접 작성 → `node scripts/shoot.cjs <name>` 렌더 → `<ctx.dir>/_shots/screens/*.png` Read해 픽셀 확인 → Edit 반복; cwd=repo루트·KILN_PROJECTS_ROOT 주입; 종료 후 디스크에서 화면 읽어 기존 `{files}` 계약 유지). `design.js`는 `isAgentic(model) ? buildScreensAgentic : buildScreens` **분기 하나** — 나머지(빈화면가드·shoot 게이트·verifier·trace)는 **불변**. **엔진 최종 shoot 게이트는 경로 무관 유지** = 신뢰 판정(에이전트 in-loop shoot는 자가수정용, 최종은 엔진 소유. 메모리 원칙 준수).
  - **max_turns = 소프트 경계(크래시 아님)**: claude가 `--max-turns` 도달 시 **non-zero exit**(runAgentStream reject)하지만 **작성한 화면은 디스크에 유효**. catch에서 `e.events`에 result가 있으면 복구 → `maxTurnsHit` 플래그로 반환 → `buildScreensAgentic`가 `emit('warn')` 후 **파일 살려 진행**(엔진 게이트가 판정). 파일 0개일 때만 하드 실패.
  - **E. 부트스트랩 — "첫 렌더 자동 다운로드" 구현**: `scripts/ensure-chromium.cjs`(chromium `executablePath()` 부재 감지 → playwright CLI를 `process.execPath`로 `install chromium` 1회, 진행 표시, 실패=graceful skip). shoot.cjs `main()` 상단 `loadChromium` 직전에 호출 → **서버 게이트·에이전트 툴 양쪽을 한 곳에서** 커버(트리거가 런처가 아니라 **렌더 seam** = 정확히 "첫 렌더 필요 시점"). **패키징 핵심 결정: playwright를 devDep→runtime `dependency` 승격.** 이유: shoot.cjs는 **spawn**돼 Next standalone 트레이서가 절대 못 담음 → 진짜 dependency여야 `npx @hb-kit/kiln` 사용자가 resolve. 브라우저 바이너리는 안 실음(첫 렌더 다운로드, per-user 캐시 재사용). **런처(bin/kiln.js) 변경 불필요** — 서버 cwd=pkgDir이라 `scripts/shoot.cjs`·`node_modules/playwright` 자동 해석, 캐시는 playwright 기본 위치. (메모리가 예고한 "standalone 트레이스 포함" 난점은 dependency 승격으로 더 깔끔히 해소.)
  - **검증(실측)**: sonnet·maxTurns 6·1화면 격리 테스트를 **실제로 돌려** — 에이전트가 home.html(9.9KB) 작성 → shoot 렌더 PASS → **PNG를 Read로 열어** `"선택선택"` 중복 텍스트(**CSS `::after`+HTML 겹침 = 소스엔 없고 렌더에만 보이는 버그, design-verifier가 절대 못 잡는 것**) 발견 → Edit로 자가수정. turn/tool-call/warn 이벤트 전부 발화, soft max_turns 파일 보존 확인. `tsc --noEmit` 클린 · 전 엔진 모듈 임포트 OK · `ensure-chromium` ready 감지 · shoot 8화면 PASS(ensure 경유) · `npm pack` 트리에 `scripts/ensure-chromium.cjs`+playwright dep 반영.
  - **다음: 착수 ③** — design-verify를 적대적 서브에이전트로(픽셀 vision 판정)·revise 확산 → ④ codex→gemini 단발 폴백 provider를 같은 스테이지 계약 뒤에. 상세 [[kiln-c2-agentic]].
- **2026-07-10 (착수 ③ 완료) — design-verify를 HTML 소스 판정 → 렌더 PNG 픽셀 판정으로 승격 + revise 확산.** atelier 적대적 design-verifier를 kiln 척추 위에 복원. **실제 claude-code 에이전트로 end-to-end 검증**.
  - **픽셀 검증(`judge.js` `judgeHiFiAgentic`)**: `runAgentic`으로 독립 claude-code 서브에이전트를 띄워 `_shots/screens/*.png`를 **Read 툴로 실제 픽셀을 보고** 적대적 판정. `tools:['Read','Write']` — Read로 PNG(주)+소스(deadControl·stateInert 보조), Write는 판정파일만(Edit/Bash 없음 = 검증대상 화면 불가침, 독립성 보존). 소스 판정(`judgeHiFi`)은 non-agentic·무-shot·파싱실패 폴백으로 유지.
  - **판정 전달 = 파일(실측으로 확정)**: 처음엔 "마지막 메시지에 verdict JSON"으로 했으나 — **모델이 발견 내용을 산문으로 서술**하고 JSON을 안 냄(실측). → 에이전트가 `design-verify.json`에 verdict를 **Write** → `readVerdict`가 디스크에서 읽어 `extractJSON`+`validate`(VERDICT_SCHEMA), 실패 시 메시지 텍스트 폴백, 그래도 실패면 throw(design.js가 소스 판정으로 폴백). buildScreensAgentic이 화면을 파일로 쓰는 것과 **동일 패턴**(디스크가 결정론적 계약).
  - **공유 헬퍼 추출(`engine/pipeline/verify.js` `verifyScreens`)**: shoot는 caller가 먼저 실행(PNG 생성), 헬퍼는 **판정만**. pixel-first(`isAgentic(judge)&&hasShots`) + 소스 폴백 + `reconcile`(빌드에 없는 환각 화면 제거) + `design-verify.md` 작성(근거 라벨=픽셀/소스) + `design-verifier` 게이트 emit — **advisory·불차단**. `design.js`는 인라인 검증(reconcile/verifyDoc/verdictSummary 포함)을 `verifyScreens` 한 줄 호출로 대체.
  - **revise 확산(`revise.js`)**: 개정 화면 재생성 후 `rebuilt[]` 수집 → `runGate('shoot.cjs')` + `verifyScreens({files:rebuilt, judge:planner, scope:'(개정)'})`. **개정도 초기 빌드와 같은 렌더+픽셀 안전망**(이전엔 shoot·verify 둘 다 없어 개정 결과가 무검증이었음).
  - **검증(실측)**: 예제 프로젝트(4화면)로 `judgeHiFiAgentic`·`verifyScreens` 각각 end-to-end — 에이전트가 4 PNG 픽셀 확인 + 소스 교차검증. 한 실행에서 **소스로는 안 보이는 vote 화면 왕관 렌더 결함**(`.crown` 기본 `display` 미설정 → 1위가 아니라 지는 후보들에 왕관 렌더)을 **픽셀에서 FAIL(bad)로 포착** — 픽셀 판정의 실효 증명. (다른 실행은 PASS = 판정 모델 변동성, advisory라 무해.) tsc 클린. env: `KILN_VERIFY_MAX_TURNS`(기본 8). **미실측 표면**: 전체 revise 런(plan→rebuild→shoot→verify) 통합은 단위 조각만 검증(shoot=design.js와 동일 경로, verify=verifyScreens 실측).
  - **다음: 착수 ④** — codex→gemini 단발 폴백 provider를 같은 스테이지 계약 뒤에 하나씩. 상세 [[kiln-c2-agentic]].
- **2026-07-10 (착수 ④ codex — 배선검증·계정 한계 문서화) — codex 단발 폴백 provider가 스테이지 계약을 정상 통과함을 확인. 코드 변경 0.** ④ "같은 스테이지 계약 뒤에 단발 폴백"의 핵심 발견: **하류가 이미 provider-무관이라 codex는 코드 추가 없이 흐른다.**
  - **왜 코드 변경이 없나**: `design.js`는 build 지점에서만 분기(`isAgentic(model) ? buildScreensAgentic : buildScreens`)하고, 그 하류(디스크 기록 → 외부 shoot 게이트 → `verifyScreens`)는 전부 provider-무관. codex는 `supportsAgentic` 없음 → `isAgentic(codex)=false` → **단발 `buildScreens`(`<<<FILE…>>>` 파싱) + 외부 shoot + 소스 판정**(`bin/forge.js`가 `judge=model=codex`라 verify도 non-agentic → `judgeHiFi`)로 자동 흐름. ② 청사진의 "codex/echo 미구현 → 단발+외부게이트 폴백"이 이미 성립.
  - **실측(`node bin/forge.js "…" --model codex`)**: 파이프라인이 codex provider에 정상 도달 → 모델 전달 → codex가 에러 반환 → **provider가 실제 사유를 그대로 표면화**(`codex: The 'gpt-5.3-codex' model is not supported when using Codex with a ChatGPT account.`). 즉 **배선·에러핸들링(② 개선)은 정상**. 이 계정에서 시도한 모든 모델(gpt-5.3-codex/gpt-5-codex/gpt-5.1-codex/gpt-5/o4-mini) 거부 → **green 완주는 이 ChatGPT 계정 플랜의 Codex 모델 접근 제한 때문에 막힘**(kiln 코드 문제 아님). auth는 ChatGPT 로그인(`~/.codex/auth.json`에 API키도 있으나 auth_mode=ChatGPT).
  - **결정(사용자 승인)**: codex는 **배선검증으로 인정·진행**(green 실측은 Codex 접근 가능한 계정/머신에서 나중에). **gemini는 이월**(CLI 미설치 — BYO gemini-cli로 추가 예정, codex.js 미러링). ④의 provider 계약은 하류 provider-무관성으로 이미 충족.
  - **다음: ④ 잔여** — Codex 가능 계정에서 codex green 실측 · gemini-cli BYO provider(provider+picker+config, codex 미러) 추가·실측. 상세 [[kiln-c2-agentic]].
