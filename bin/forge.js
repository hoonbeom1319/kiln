...
//
//   node bin/forge.js "<아이디어>" [--name slug] [--model claude-code|codex] [--judge …]
...
async function main() {
  ...
  if (!idea) { console.error('사용법: node bin/forge.js "<아이디어>" [--name slug] [--model claude-code|codex] [--judge …]'); process.exit(1); }
  ...
  const r = await forge(ctx, { emit, model: agent, judge: args.judge || agent });
