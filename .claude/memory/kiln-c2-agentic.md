...
**착수 순서(하나씩):** ✅ **① shoot 렌더 게이트 부활 — 완료(2026-07-10)** → ✅ **② runAgentic+stream-json 렌더-인-루프 — 완료(2026-07-10)** → ③ design-verify(적대적 서브에이전트·픽셀 vision)·revise 확산 → ④ **codex → gemini** 단발 폴백 provider를 같은 스테이지 계약 뒤에 하나씩.
...
**다음 세션은 ③부터** — design-verify를 적대적 서브에이전트로(픽셀 vision 판정, `_shots/*.png` 입력)·revise 확산. 이후 ④ codex→gemini 단발 폴백 provider(같은 스테이지 계약 뒤). agentic 시임(`runAgentic`/`buildScreensAgentic`)은 완비 — ③은 verify/revise 쪽에 같은 패턴 적용.
